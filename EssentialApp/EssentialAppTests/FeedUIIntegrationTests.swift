...
class FeedUIIntegrationTests: XCTestCase {
    ...
    func test_loadFeedActions_requestFeedFromLoader() {
        ...
        XCTAssertEqual(loader.loadFeedCallCount, 1, "Expected no request intil previous completes")
    ...
    func test_loadingMoreIndicator_isVisibleWhileLoadingMore() {
        ...
        loader.completeLoadMoreWithError(at: 1)
        XCTAssertFalse(sut.isShowingLoadMoreFeedIndicator, "Expected no loading indicator once user initiated loading completes with erorr")
    }
    ...
    func test_errorView_rendersErrorMessageOnLoadFeedError() {
        let (sut, loader) = makeSUT()
    ...
    func test_feedImageViewLoadingIndicator_isVisibleWhileLoadingImage() {
        ...
        XCTAssertEqual(view1?.isShowingImageLoadingIndicator, true, "Expected loading indicator for first second while loading second image")
    ...
    func test_feedImageViewRetryButton_isVisibleOnimageURLLoadError() {
        ...
        XCTAssertEqual(view1?.isShowingRetryAction, false, "Expected no retyr action for second view while loading second image")
    ...
    func test_feedImageViewRetryAction_retriesImageLoad() {
        ...
        let view0 = sut.simulateFeedImageViewVisible(at: 0)
        let view1 = sut.simulateFeedImageViewVisible(at: 1)
        XCTAssertEqual(loader.loadedImageURLs, [image0.url, image1.url], "Expected two image URL request fr the two visible views")
    ...
    func test_feedImageView_cancelsImageURLPreloadingWhenNotNearVisibleAnymore() {
        ...
        XCTAssertEqual(loader.cancelledImageURLs, [image0.url, image1.url], "Epxected second cancelled image URL request once second image is not near visible anymore")
    }
    ...
    func assertThat(_ sut: ListViewController, isRendering feed: [FeedImage], file: StaticString = #filePath, line: UInt = #line) {
        ...
        feed.enumerated().forEach { index, image in
            asserThat(sut, hasViewConfiguredFor: image, at: index, file: file, line: line)
        }
    }
    
    func asserThat(_ sut: ListViewController, hasViewConfiguredFor image: FeedImage, at index: Int, file: StaticString = #filePath, line: UInt = #line) {
        ...
        XCTAssertEqual(cell.isShowingLocation, shouldBeVisible, "Epxected `isShowingLocation` to be \(shouldBeVisible) for image view at index (\(index))",file: file, line: line)
